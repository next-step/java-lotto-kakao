...
public enum Prize {
    ...
    /**
     * 게임 결과에 해당하는 당첨 등급을 반환합니다. * @param gameResult 로또 비교 결과
     *
     * @return 일치하는 {@link Prize}, 없으면 {@link #NONE}
     * @implNote 선언된 순서대로 검사를 진행하므로, 보너스 번호 일치가 필요한 등급이 일반 일치 등급보다 우선순위가 높아야 합니다.
     */
    ...
    private boolean matches(GameResult result) {
        return result.matches(matchingCount, bonusMatchingCount);
    }
