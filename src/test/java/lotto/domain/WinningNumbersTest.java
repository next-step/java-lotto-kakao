...
class WinningNumbersTest {
	...
	@DisplayName("보너스 번호가 당첨 번호와 중복되면 LottoValidationException이 발생해야 한다")
	@Test
	void of_withDuplicateBonusNumber_throwsLottoValidationException() {
		Lotto numbers = Lotto.from(List.of(1, 2, 3, 4, 5, 6));
	...
	@DisplayName("당첨 번호가 null이면 LottoValidationException이 발생해야 한다")
	@Test
	void of_withNullNumbers_throwsLottoValidationException() {
		assertThatThrownBy(() -> WinningNumbers.of(null, LottoNumber.from(7)))
			.isInstanceOf(LottoValidationException.class);
	}

	@DisplayName("보너스 번호가 null이면 LottoValidationException이 발생해야 한다")
	@Test
	void of_withNullBonusNumber_throwsLottoValidationException() {
		Lotto numbers = Lotto.from(List.of(1, 2, 3, 4, 5, 6));
