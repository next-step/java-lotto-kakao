...
class LottoMachineTest {
	...
	@DisplayName("자동 발급 계산 시 금액이 가격 미만이면 IllegalArgumentException이 발생해야 한다")
	@Test
	void calculateRandomCountFromAmount_withAmountLessThanPrice_throwsIllegalArgumentException() {
		LottoMachine machine = new LottoMachine();

		assertThatIllegalArgumentException()
			.isThrownBy(() -> machine.calculateRandomCountFromAmount(999, 0));
	}
	...
	@DisplayName("자동 발급 계산 시 수동 개수가 음수면 IllegalArgumentException이 발생해야 한다")
	@Test
	void calculateRandomCountFromAmount_withNegativeManualCount_throwsIllegalArgumentException() {
		LottoMachine machine = new LottoMachine();

		assertThatIllegalArgumentException()
			.isThrownBy(() -> machine.calculateRandomCountFromAmount(1_000, -1));
	}

	@DisplayName("자동 발급 계산 시 수동 개수가 전체 발급 개수를 초과하면 IllegalArgumentException이 발생해야 한다")
	@Test
	void calculateRandomCountFromAmount_withManualCountExceedingTotal_throwsIllegalArgumentException() {
		LottoMachine machine = new LottoMachine();

		assertThatIllegalArgumentException()
			.isThrownBy(() -> machine.calculateRandomCountFromAmount(1_000, 2));
	}
